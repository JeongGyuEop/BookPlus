...
$(document).ready(function() {
    $('#checkHoroscopeBtn').click(function() {
        var sign = $('#sign').val();  // 선택된 별자리 값을 가져옴
        var language = $('#language').val();  // 선택된 언어 값 가져오기
        ...
        $.ajax({
            ...
            success: function(response) {
                // 운세가 정상적으로 반환되었으면 운세 내용 표시
                if (response && response.horoscope) {
                    var horoscopeText = response.horoscope;  // 응답에서 운세 내용 가져오기
                    
                    // 선택된 언어가 있을 경우 번역
                    if (language === "ko") {
                        translateText(horoscopeText, "ko");  // 한국어로 번역
                    } else {
                        $('#horoscopeDescription').html(horoscopeText);  // 영어로 그대로 표시
                    }
                } else {
                    $('#horoscopeDescription').html("운세를 가져올 수 없습니다.");
                }
...
//언어 변경 시 자동 번역 처리
 // Google Translate API를 사용한 번역 함수
    function translateText(text, targetLanguage) {
        ...
        const apiKey = 'AIzaSyAmnrrbVyftChKEk44vDuoCVR4vfCXLRuY';  // 여기에 본인의 Google API 키를 넣어주세요
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response && response.data && response.data.translations[0]) {
                    var translatedText = response.data.translations[0].translatedText;
                    $('#horoscopeDescription').html(translatedText);
                } else {
                    $('#horoscopeDescription').html("번역 결과를 가져올 수 없습니다.");
                }
            },
            error: function (xhr, status, error) {
                console.error("Translation API error:", xhr.responseText);
                $('#horoscopeDescription').html("번역 요청 중 오류가 발생했습니다.");
            }
        });
    }
 */
/*
    ...
    document.getElementById('languageSelect').addEventListener('change', function () {
        var selectedLanguage = this.value; // 사용자가 선택한 언어 코드 (예: 'ko' 또는 'en')를 가져옴
        var originalText = document.getElementById('horoscopeDescription').innerText; // 번역 대상 텍스트를 가져옴
