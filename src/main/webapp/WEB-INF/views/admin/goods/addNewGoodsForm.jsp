...
<script type="text/javascript">
  ...
  var cnt=0; // 상세 이미지의 첨부 순서를 나타냅니다. 
  
  //아래의 파일추가 버튼을 클릭하면 호출되는 함수로  
  //첨부할 파일을 선택할 <input>들을 동적으로 생성하여  
  //id속성값이 d_file인 <div id="d_file"></div>요소 내부에 추가시키는 함수 
  function fn_addFile(){
	  if(cnt == 0){
		  //첫번째 파일업로드는 메인 이미지를 첨부하므로 name속성값을 main_image로 설정합니다. 
		  $("#d_file").append("<br>"+"<input  type='file' name='main_image' id='f_main_image' />");	  
	  }else{
		  //그외의 업로드하는 파일 이미지들은  name속성값들을 detail_image+cnt 로 설정합니다. 
...
<div class="tab_container">
	...
	<div class="tab_container" id="container">
		...
		<div class="tab_container">
			...
			<%--
				 상품이미지를 여러개 첨부할 경우 첫번쨰 파일 업로드의 name속성값을 main_image로 설정합니다.
				 그러면 테이블에 저장되는 file_type속성이 main_image로 저장됩니다.
				 그외 파일은 detail_image로 저장됩니다. 
			 --%>
