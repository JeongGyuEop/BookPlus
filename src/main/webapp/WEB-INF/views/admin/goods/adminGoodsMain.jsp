...
<TABLE class="list_view">
		<TBODY align=center >
			<tr style="background:#33ff00" >
				<td>상품ISBN</td>  <!-- goods_id → goods_isbn -->
				<td>상품이름</td>
				<td>저자</td>      <!-- goods_writer → goods_author -->
				<td>출판사</td>
				<td>상품판매가격</td> <!-- goods_sales_price → goods_priceSales -->
				<td>입고일자</td>
				<td>출판일</td>    <!-- goods_published_date → goods_pubDate -->
			</tr>
   <c:choose>
     ...
     <c:forEach var="item" items="${newGoodsList}">
			 <TR>       
				<TD>
				  <strong>${item.goods_isbn }</strong> <!-- changed goods_id → goods_isbn -->
				</TD>
				<TD >
				 <a href="${pageContext.request.contextPath}/admin/goods/modifyGoodsForm.do?goods_isbn=${item.goods_isbn}">
				    <strong>${item.goods_title} </strong>
				 </a> 
				</TD>
				<TD>
				<strong>${item.goods_author }</strong> <!-- changed goods_writer → goods_author -->
				</TD>
				<TD >
				   <strong>${item.goods_publisher }</strong> 
				</TD>
				<td>
				  <strong>${item.goods_priceSales }</strong> <!-- changed goods_sales_price → goods_priceSales -->
				</td>
				<td>
				 <strong>${item.goods_credate }</strong> 
				</td>
				<td>
				    <c:set var="pub_date" value="${item.goods_pubDate}" /> <!-- changed goods_published_date → goods_pubDate -->
