...
/*    
	회원 정보 수정페이지를 최초로 로드할때 자바스크립트 함수를 사용해 
	유선전화번호 와 휴대폰의 국번의 select option태그에 접근하여 회원의 국번으로 설정(초기화)합니다.
*/
   ...
   window.onload=function()
    ...
    //유선전화의 국번을 저장합니다. 
     ...
     //유선전화번호의 국번을 선택하는 <select><option></option></select> 요소를 선택해서 변수에 저장
     var selTel1 = document.getElementById('tel1');
     //휴대폰의 국번을 선택하는 <select><option></option></select> 요소를 선택해서 변수에 저장    
     var selHp1 = document.getElementById('hp1');
     
     //유선전화 국번과 휴대폰 국번을 선택하는  <option>태그들을 배열에 담아 선택해 옵니다. 
		...
		<table>
			<tbody>
				<tr class="dot_line">
					...
					<td>
						<input name="member_id" type="text" size="20" value="${sessionScope.memberInfo.member_id }"  disabled/>
					</td>
					 <td>
					</td>
				</tr>
